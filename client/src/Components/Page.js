...
const AddEmployees = () => {
    axios
      ...
      .then((response) => {
        setEmployeeList([...employeelist,{ name: name,
          age: age,
          country: country,
          position: position,
          wage: wage,}]);
...
const deleteEmployee =(id)=>{
  ...
  .then((response)=>{
    setEmployeeList(employeelist.filter((e)=>{
      return e.id !== id  
    }))
  })
}
...
<div>
  <Appinput type='text' onChange={(e) => {
            setNewPosition(e.target.value);
          }}  />
          <Appbutton onClick={()=>{
            updateEmployeePosition(e.id)
          }}>UPDATE POSITION</Appbutton>
